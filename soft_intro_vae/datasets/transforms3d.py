...
class RandomRotateAxisAngle(RotateAxisAngle):
    def __init__(
            self,
            bsz: int = 1,
            axis: str = "X",
        ...
        angle = torch.rand(size=(bsz,)) * 90 - 45
        super(RandomRotateAxisAngle, self).__init__(angle, axis=axis, degrees=degrees, dtype=dtype, device=device)
...
class RandomTranslate(Translate):
    def __init__(
        ...
        xyz = torch.rand(size=(bsz, 3)) * range - range / 2.
        xyz[:, 2] = 0.
        super(RandomTranslate, self).__init__(xyz, dtype=dtype, device=device)
